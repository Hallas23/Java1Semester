...
public class TilmeldingPane extends GridPane {
	private TextField txfName, txfLedsager, txfKonference, txfHotel, txfAnkomstdato, txfAfrejsedato, txfForedrag,
			txfDobbeltseng, txfMad, txfWifi, txfBad;
	...
	public TilmeldingPane() {
		...
		this.add(lvwtilmeldinger, 0, 1, 1, 17);
		...
		this.add(hbxButtons, 0, 18, 3, 1);
		hbxButtons.setPadding(new Insets(10, 0, 0, 0));
		hbxButtons.setAlignment(Pos.TOP_LEFT);
	...
	public void updateTilmelding() {
		...
		if (tilmelding != null) {
			...
			for (Udflugt ud : tilmelding.getUdflugter()) {
				sb.append(ud + "\n");
			}
			lvwUdflugter.getItems().setAll(tilmelding.getUdflugter());

			if (tilmelding.isForedragsholder()) {
				txfForedrag.setText("True");
			} else {
				txfForedrag.setText("false");
			}

			if (tilmelding.isSeng()) {
				txfDobbeltseng.setText("True");
			} else {
				txfDobbeltseng.setText("false");
			}
