package guifx;

import application.controller.Controller;
import application.model.Hotel;
import javafx.geometry.HPos;
import javafx.geometry.Insets;
import javafx.scene.Scene;
import javafx.scene.control.Button;
import javafx.scene.control.Label;
import javafx.scene.control.TextField;
import javafx.scene.layout.GridPane;
import javafx.stage.Modality;
import javafx.stage.Stage;
import javafx.stage.StageStyle;

public class HotelWindow extends Stage {
	private Hotel hotel;

	public HotelWindow(String title, Hotel hotel) {
		this.initStyle(StageStyle.UTILITY);
		this.initModality(Modality.APPLICATION_MODAL);
		this.setResizable(false);

		this.hotel = hotel;

		this.setTitle(title);
		GridPane pane = new GridPane();
		this.initContent(pane);

		Scene scene = new Scene(pane);
		this.setScene(scene);
	}

	public HotelWindow(String title) {
		this(title, null);
	}

	// -------------------------------------------------------------------------

	private TextField txfName, txfAdresse, txfBadpris, txfMadpris, txfWifipris;
	private Label lblError;

	private void initContent(GridPane pane) {
		
		pane.setPadding(new Insets(10));
		pane.setHgap(10);
		pane.setVgap(10);
//		pane.setGridLinesVisible(true);

		Label lblName = new Label("Navn");
		pane.add(lblName, 0, 0);
		txfName = new TextField();
		pane.add(txfName, 1, 0);
		txfName.setPrefWidth(200);

		Label lblAdresse = new Label("Adresse");
		pane.add(lblAdresse, 0, 1);
		txfAdresse = new TextField();
		pane.add(txfAdresse, 1, 1);
		txfAdresse.setPrefWidth(200);

		Label lblBadpris = new Label("Badpris");
		pane.add(lblBadpris, 0, 2);
		txfBadpris = new TextField();
		pane.add(txfBadpris, 1, 2);
		txfBadpris.setPrefWidth(200);

		Label lblMadpris = new Label("Madpris");
		pane.add(lblMadpris, 0, 3);
		txfMadpris = new TextField();
		pane.add(txfMadpris, 1, 3);
		txfMadpris.setPrefWidth(200);

		Label lblWifipris = new Label("Wifipris");
		pane.add(lblWifipris, 0, 4);
		txfWifipris = new TextField();
		pane.add(txfWifipris, 1, 4);
		txfWifipris.setPrefWidth(200);

		// ----------------------------


		Button btnCancel = new Button("Cancel");
		pane.add(btnCancel, 0, 7);
		GridPane.setHalignment(btnCancel, HPos.LEFT);
		btnCancel.setOnAction(event -> this.cancelAction());

		Button btnOK = new Button("OK");
		pane.add(btnOK, 1, 7);
		GridPane.setHalignment(btnOK, HPos.RIGHT);
		btnOK.setOnAction(event -> this.okAction());

		lblError = new Label();
		pane.add(lblError, 0, 8);
		lblError.setStyle("-fx-text-fill: red");

		this.initControls();
	}

	private void initControls() {
		if (hotel != null) {
			txfName.setText(hotel.getNavn());
			txfAdresse.setText("" + hotel.getAdresse());
		} else {
			txfName.clear();
			txfAdresse.clear();
		}
	}

	// -------------------------------------------------------------------------

	private void cancelAction() {
		this.hide();
	}

	private void okAction() {
		String navn = txfName.getText().trim();

		if (navn.length() == 0) {
			lblError.setText("Name is empty");
			return;
		}
		String adresse = txfAdresse.getText().trim();
		if (adresse.length() == 0) {
			lblError.setText("Name is empty");
			return;
		}

		double Badpris = Integer.parseInt(txfBadpris.getText().trim());

		if (Badpris < 0) {
			lblError.setText("Name is empty");
			return;
		}

		double Madpris = Integer.parseInt(txfMadpris.getText().trim());

		if (Madpris < 0) {
			lblError.setText("Name is empty");
			return;
		}

		double wifipris = Integer.parseInt(txfWifipris.getText().trim());

		if (wifipris < 0) {
			lblError.setText("Name is empty");
			return;
		}

	}

}
